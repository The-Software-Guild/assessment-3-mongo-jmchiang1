...
const BugForm = () => {
  ...
  useEffect(() => {
    if (current !== null) {
      //if current has no data, set bug to new data
      setBug(current);
    } else {
      //else clear bug data back to empty string
      setBug({
        name: "",
        description: "",
      });
  ...
  const [bug, setBug] = useState({
    //initialize bug object to empty strings
    name: "",
    description: "",
  });
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    if (current === null) {
      //if form has no data inside, add bug
      addBug(bug);
    } else {
      //else update it and clear bug state
      updateBug(bug);
      setBug({
        name: "",
        description: "",
  ...
  return (
    <form onSubmit={onSubmit} className="bug-form">
      ...
      {/* Submit Button */}
      <div>
        <button type="submit">{current ? "Update Bug" : "Add Bug"}</button>
      </div>
    </form>
  );
};
